...
function getSearchSpecialsQuery(evt) {
  ...
  if (evt.target.id === "searchSpecials") return;     
    ...
    special = evt.target.textContent.trim();    
  else special = evt.target.parentNode.textContent.trim();

    results = specialSearch(families, special);  
  loadSearchResults(results);
}
...
function specialSearch(families, special) {
  ...
  special = html2json[special];  
    ...
    if (special) {
        ...
        if (currentCountry === "SAM") return searchExtinctOrEndemicSAM(families, special);    
...
function searchRegexTree(families, query, country, modifyBoolean) {
  ...
  if (modifyBoolean)query = modifyQuery(query);
    ...
    return searchAllQuery(families, query, country);                
}
  ...
  var familyAdded;
    ...
    familyAdded = false;
        ...
        for (var genus of family.genera)  {
            ...
            var birdsWithSpecial = genus.spp.filter(function(bird) {    
...
function getAllSpeciesFromOneFamily($results, thisFamily, country) {
  ...
  for (var family of families) {  
    ...
    if (!(regex.test(family.Family))) continue;  
...
function toggleSearchInstructions() {
  ...
  if (searchInstructionsOpen) {
    ...
    toggleFooterFocusable(true);  
  }
  else {
    moveTaxPanel("searchInstructionsOpening");
    toggleFooterFocusable(false);  
  }
...
function gotoMatch(e) {
  ...
  for (var i = 0; i < sLen; i++) {
    ...
    if (entryTextTrimmed === eText && clickedClass !== "family") {
      ...
      elem = simpleBarTaxPage.getScrollElement();
      elem.scroll({top: entry.offsetTop, behavior: 'smooth'});  
